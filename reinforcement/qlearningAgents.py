...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeActionFromQValues(self, state):
        ...
        """
        "*** YOUR CODE HERE ***"
        maximum = self.computeValueFromQValues(state)
        actions = self.getLegalActions(state)
        if len(actions)==0:
            return None
        maxAct = []
        for action in actions:
            if (self.getQValue(state, action) == maximum):
                maxAct.append(action)
    ...
    def update(self, state, action, nextState, reward):
        ...
        sample = reward + self.discount*(next)
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        next = self.getValue(nextState)
        features = self.featExtractor.getFeatures(state, action)
        difference = (reward + self.discount * next) - self.getQValue(state, action)
        for feature in features:
