...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def __init__(self, mdp, discount = 0.9, iterations = 100):
        ...
        for i in range(iterations):
            ...
            for state in self.mdp.getStates():
                ...
                values = [self.computeQValueFromValues(state, act) for act in actions]
                current_values[state] = max(values)
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for i in transitionStatesAndProbs:
            r = self.mdp.getReward(state, action, i[0])
            v = self.values[i[0]]
            answer = answer + (r + self.discount * v)* i[1]
